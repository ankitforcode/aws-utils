...
// Instance : This return the details of currently running instance
// regardless of the region since that should be passed with the
// aws session client when calling the method.
...
// Members : This returns the list of the instances that are currently member
// of similar tags. Since we are more interested in sorting the details
// by launch time.
func (s *Cluster) Members() ([]*ec2.Instance, error) {
	tagValue := s.TagValue
	// Get the Tag of the instance if not already set.
	if tagValue == "" {
...
// AutoscalingGroup : This returns the autoscaling group details
// based on the instance passwd. Assuming that the instance is
// part of autoscaling group.
...
// DiscoverInstanceID : This returns the instance id for
// the running instance also can act as a validation for
// the instance currenly running / stopped.
